...
2. We need to create relationships between models. So that we can access users associated with comments and vice versa.
    ...
    - back_populates: Defines what we should use to access a users on specific comments
...
class User(Base):
    ...
    # We want this to be a list of comment objects. 
...
class Comment(Base):
    ...
    # Define a single user object that the comment is linked to
    user:Mapped["User"]